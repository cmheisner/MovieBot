"""
Seed script — bulk-add This Fall movies to the stash.

Usage (from the repo root):
    python scripts/seed_fall.py

Requires the .env file to be present (for OMDB_API_KEY and DB_PATH).
Movies already in the DB are skipped silently (group tag is applied if missing).
"""
...
FALL_MOVIES: list[tuple[str, int]] = [
    ("28 Days Later", 2002),
    ("28 Weeks Later", 2007),
    ("28 Years Later", 2025),
...
async def main() -> None:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        async with aiohttp.ClientSession() as session:
            added = skipped = 0
            for search_title, year in FALL_MOVIES:
                omdb = await fetch_omdb(session, search_title, year)
                # Use OMDB's canonical title/year if found, otherwise fall back to provided values
